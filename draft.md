...
(using the '-' option to read the input from stdin). Note that starting address
`0x80001000` needs to be specified because the raw binary file does not contain
this metadata. Without it, branch offsets and jump targets would not be decoded
correctly.
